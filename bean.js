...
!function (name, context, definition) {
                ...
                forAll = function (element, type, original, handler, fn) {
                    if (!type || type === '*') {
                        ...
                        for (l = list.length; i < l; i++) {
                            if (all || list[i].matches(element, original, handler)) if (!fn(list[i], list, i, type)) return
                        }
                    }
                }
    ...
    , removeListener = function (element, orgType, handler, namespaces) {
        ...
        for (i = 0, l = handlers.length; i < l; i++) {
            if (handlers[i].inNamespaces(namespaces)) {
                if ((entry = handlers[i])[eventSupport]) listener(entry[targetS], entry.eventType, entry.handler, false, entry.type)
    ...
    , addListener = function (element, orgType, fn, originalFn, args) {
        ...
        if (customEvents[type]) {
            if (customEvents[type].condition) fn = customHandler(element, fn, type, customEvents[type].condition, args, true);
        ...
        if (entry[eventSupport]) listener(entry[targetS], entry.eventType, entry.handler, true, entry.customType);
    }
    ...
    , clone = function (element, from, type) {
        ...
        for (; i < l; i++) {
            if (handlers[i].original) {
                ...
                else args = [element, handlers[i].type, handlers[i].original]
                add.apply(null, args)
            }
        }
        return element
